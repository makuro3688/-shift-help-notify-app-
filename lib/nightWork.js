 ...
 *
 * 方針（優先順位つき）：
 * 1. 開始・終了の2つの時刻が抽出でき、信頼できる区間として計算できた場合は、
 *    区間が深夜帯と重なるかどうかの判定結果を「そのまま」最終結果とする。
 *    区間が「深夜にかからない」と確定しているのに、文中に「深夜」「翌」「早朝」等の
 *    キーワードが（無関係な文脈で）含まれるだけで警告を上書きすることはしない。
 *    例：「18:00〜22:00（早朝勤務ではありません）」は警告しない。
 * 2. 時刻が1つしか読み取れない場合は、現行どおりその時刻単体が深夜帯に属するか、
 *    またはキーワードの有無で判定する。
 * 3. 時刻がまったく読み取れない場合は、キーワード（深夜・翌・早朝）の有無でのみ判定する。
 * 4. パースできず、キーワードもない場合は false を返す（警告を出さない）。
 *    毎回警告を出すと「またか」と無視されるようになり、かえって危険なため、
 *    自信を持って深夜と判定できる場合にのみ警告対象とする。
 *    キーワードはあくまで「時刻が読み取れないときの補助」であり、
 *    区間判定ができる場合はキーワードより区間判定を優先する。
...
function isNightWork(timeText) {
  ...
  // 開始・終了の2つの時刻が抽出でき、信頼できる区間として計算できた場合は、
  // レンジ判定の結果を最終結果とする（キーワードによる上書きはしない）。
  // 例:「18:00〜22:00（早朝勤務ではありません）」のように、区間としては深夜にかからないのに
  // 「早朝」等のキーワードが文中に含まれるだけで警告してしまう過剰警告を防ぐため。
  // キーワードは「時刻がまったく読み取れない場合」の補助（上のtokens.length===0の分岐）に限定する。
  return rangeOverlapsNight(startMin, endMin);
}
