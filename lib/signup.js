...
// 方式に変更する。判定の大半はSQL側(supabase/setup.sqlのconsume_signup_attempt)に移り、
// ここでの責務は「RPCが返した行に対するハッシュ比較のみ」に縮小する。

// consume_signup_attempt RPCが返した行（＝試行枠を消費できた場合のみ非nullで返る）に対して、
// 入力コードが一致するかどうかだけを判定する純粋関数。
// 存在確認・期限切れ確認・試行回数の上限判定はすべてSQL側（単一のUPDATE...WHERE...RETURNING文）
// で完了しているため、この関数がfalseを返す経路は「pendingがそもそも無い(=枠を消費できなかった。
// 該当なし／期限切れ／上限到達のいずれか)」か「ハッシュが不一致」のいずれかのみ。
function isSignupCodeMatch({ pending, code, hashCode }) {
  return Boolean(pending) && pending.code_hash === hashCode(code);
}

// pending_signupsに対する確認コードの検証を、原子的な試行枠消費とあわせて行う。
// server.js側の役割は、この結果（ok/pending）に応じて店舗作成・pending行の後始末を
// 行うことのみに限定する。
//
// 引数:
...
// 戻り値:
//   { ok: true, pending }  … 検証成功。呼び出し側は店舗を作成し、pending行を削除してよい。
...
async function verifySignupCode({ supabase, email, code, hashCode, maxAttempts = SIGNUP_CODE_MAX_ATTEMPTS }) {
  // 【原子性がここでの肝】consume_signup_attempt は単一のSQL文（UPDATE...RETURNING）であり、
  // Postgresの行ロックにより「存在確認・期限切れ確認・上限確認・試行回数の加算」が
  // 不可分に実行される。同時に何件リクエストが届いても、消費に成功する（＝照合に進める）
  // 回数は1コードあたり最大maxAttempts回に厳密に制限される（並行数に依存しない）。
  ...
  if (!isSignupCodeMatch({ pending, code, hashCode })) {
    return { ok: false };
  }
