...
// AC-M3-11〜14: 過剰警告の是正（レンジが確定している場合はキーワードで上書きしない）
//
// 開始・終了の2つの時刻が抽出でき、区間として深夜にかからないと判定できているのに、
// 文中の「深夜」「翌」「早朝」等のキーワードにより警告が上書きされてしまう過剰警告バグの
// 回帰テスト。キーワードは「時刻がまったく読み取れない場合」の補助に限定されるべきで、
// 区間判定が確定している場合はそれを優先しなければならない。

test('AC-M3-11・正常系: "18:00〜22:00（早朝勤務ではありません）" はレンジが深夜にかからないため警告しない（「早朝」キーワードでの上書き禁止）', () => {
  assert.strictEqual(isNightWork('18:00〜22:00（早朝勤務ではありません）'), false);
});

test('AC-M3-12・正常系: "9:00〜17:00 翌月から時給改定" はレンジが深夜にかからないため警告しない（「翌」キーワードでの上書き禁止）', () => {
  assert.strictEqual(isNightWork('9:00〜17:00 翌月から時給改定'), false);
});

test('AC-M3-11派生・異常系: 「早朝」の文言があっても、レンジが実際に深夜（5時未満）にかかる場合は引き続き警告する', () => {
  assert.strictEqual(isNightWork('4:00〜9:00（早朝勤務ではありません）'), true);
});

test('AC-M3-12派生・異常系: 「翌」の文言があっても、レンジが実際に深夜にかかる場合は引き続き警告する', () => {
  assert.strictEqual(isNightWork('21:00〜23:00 翌月から時給改定'), true);
});

test('AC-M3-13・異常系: "深夜シフト"（時刻なし・キーワードのみ）は引き続き警告する', () => {
  assert.strictEqual(isNightWork('深夜シフト'), true);
});

test('AC-M3-13派生・正常系: 時刻がまったく読み取れず、キーワードもない場合は警告しない', () => {
  assert.strictEqual(isNightWork('通常勤務、シフト希望者募集中'), false);
});

test('AC-M3-14・異常系: "22:00〜翌2:00" はレンジが深夜にかかるため引き続き警告する', () => {
  assert.strictEqual(isNightWork('22:00〜翌2:00'), true);
});

test('AC-M3-14派生・正常系: レンジが深夜にかからず、かつキーワードもない場合は警告しない（回帰確認）', () => {
  assert.strictEqual(isNightWork('9:00〜17:00'), false);
});
